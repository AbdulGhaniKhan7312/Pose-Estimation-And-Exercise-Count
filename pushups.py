...
def draw_2points(image, p1, p2, frame_width, frame_height, color1, color2):
    ...
    cv2.line(image, tuple(np.multiply([p1.x, p1.y], [frame_width,frame_height]).astype(int)), tuple(np.multiply([p2.x, p2.y], [frame_width,frame_height]).astype(int)), color1,3)
    cv2.circle(image, tuple(np.multiply([p1.x, p1.y], [frame_width,frame_height]).astype(int)), 5, color2, cv2.FILLED)
    cv2.circle(image, tuple(np.multiply([p1.x, p1.y], [frame_width,frame_height]).astype(int)), 10, color2, 2)
    cv2.circle(image, tuple(np.multiply([p2.x, p2.y], [frame_width,frame_height]).astype(int)), 5, color2, cv2.FILLED)
    cv2.circle(image, tuple(np.multiply([p2.x, p2.y], [frame_width,frame_height]).astype(int)), 10, color2, 2)
...
#     disp_feedback_1 = 'KEEP WRIST STRAIGHT'
    disp_feedback_2 = 'Keep back straight'
    disp_feedback_3 = 'Keep knee straight'
...
#     pTime = 0
    ...
    while cap.isOpened():
        ...
        try:
            ...
            if left_hip_angle < hip_thresh[0] and right_hip_angle < hip_thresh[0] :
                image = draw_points(image,left_shoulder,left_hip,left_knee,frame_width,frame_height,RED,WHITE)
                image = draw_points(image,right_shoulder,right_hip,right_knee,frame_width,frame_height,RED,WHITE)
                draw_text(
                        image, 
                        disp_feedback_2, 
            ...
            if left_knee_angle < knee_thresh[0] and right_knee_angle < knee_thresh[0] : 
                ...
                if left_elbow_angle > elbow_thresh[1] and right_elbow_angle > elbow_thresh[1]:
                    count_flag = 'up'
                if left_elbow_angle < elbow_thresh[0] and right_elbow_angle < elbow_thresh[0] and count_flag=='up':
                    if count == reps:
                        count_complete = True
                    else:
                        count += 1
...
if __name__=='__main__':
    ...
    engine = pyttsx3.init()   # text to speech initialise
    engine.say('Your pushup exercise is about to start, lay down, remember to maintain a good posture and keep in position.')
    engine.runAndWait()
