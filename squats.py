...
def infer():
    ...
    hip_thresh = [175, 75]
    ankle_thresh = 45
    knee_thresh = [120, 95,70]
    offset_thresh = 0
    nose_thresh = 0
    shoulder_thresh = 0
    
#     disp_feedback_0 = 'ROTATE SIDEWAYS'
    disp_feedback_1 = 'Lift your back a little'
    disp_feedback_2 = 'Bend your back forward'
    disp_feedback_3 = 'Knee is falling over toe'
    disp_feedback_4 = 'Squat is too deep'
#     disp_feedback_5 = 'BEND YOUR KNEE'
...
#     pTime = 0
    ...
    while cap.isOpened():
        ...
        try:
            ...
            left_knee_angle = int(calc_angle(left_hip,left_knee,left_ankle))
            right_knee_angle = int(calc_angle(right_hip,right_knee,right_ankle))
            
            left_ankle_angle = int(calc_angle(left_knee,left_ankle,left_foot))
            right_ankle_angle = int(calc_angle(right_knee,right_ankle,right_foot))
            
            # Render
            
            image = draw_points(image,left_hip,left_knee,left_ankle,frame_width,frame_height,BLUE,WHITE)
            image = draw_points(image,left_shoulder,left_hip,left_knee,frame_width,frame_height,BLUE,WHITE)

            image = draw_points(image,right_hip,right_knee,right_ankle,frame_width,frame_height,BLUE,WHITE)
            image = draw_points(image,right_shoulder,right_hip,right_knee,frame_width,frame_height,BLUE,WHITE)

            # Feedback

            if left_hip_angle < hip_thresh[1] and right_hip_angle < hip_thresh[1]:
                image = draw_2points(image,left_shoulder,left_hip,frame_width,frame_height,RED,WHITE)
                image = draw_2points(image,right_shoulder,right_hip,frame_width,frame_height,RED,WHITE)
                draw_text(
                    image, 
                    disp_feedback_1, 
            ...
            elif left_hip_angle > hip_thresh[0] and right_hip_angle > hip_thresh[0]:
                image = draw_2points(image,left_shoulder,left_hip,frame_width,frame_height,RED,WHITE)
                image = draw_2points(image,right_shoulder,right_hip,frame_width,frame_height,RED,WHITE)
                draw_text(
                    image, 
            ...
            if left_ankle_angle < ankle_thresh and right_ankle_angle < ankle_thresh: 
                image = draw_2points(image,left_knee,left_ankle,frame_width,frame_height,RED,WHITE)
                image = draw_2points(image,right_knee,right_ankle,frame_width,frame_height,RED,WHITE)
                draw_text(
                    image, 
                    disp_feedback_3, 
            ...
            if left_knee_angle < knee_thresh[2] and right_knee_angle < knee_thresh[2]:
                image = draw_points(image,left_hip,left_knee,left_ankle,frame_width,frame_height,RED,WHITE)
                image = draw_points(image,right_hip,right_knee,right_ankle,frame_width,frame_height,RED,WHITE)
                draw_text(
                    image, 
                    disp_feedback_4, 
                    pos=(30, frame_height-130),
                    text_color=WHITE,
                    font_scale=0.7,
                    text_color_bg= RED,
                )
                
            if left_knee_angle > knee_thresh[0] and right_knee_angle > knee_thresh[0]: 
                count_flag = 'up'
            if left_knee_angle < knee_thresh[1] and count_flag =='up' and right_knee_angle < knee_thresh[1] :
                if count == reps:
                    count_complete = True
                else:
                    count += 1
...
if __name__=='__main__':
    ...
    engine = pyttsx3.init()   # text to speech initialise
    engine.say('Your squats exercise is about to start, remember to maintain a good posture')
    engine.runAndWait()
